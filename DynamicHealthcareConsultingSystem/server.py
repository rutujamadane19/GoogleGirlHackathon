...
def main(): 
    if 'loggedin' in session:
        if request.method=='POST':
            ...
            genai.configure(api_key="AIzaSyCK5zz3ZQvu2c6ClixpnPAcSubpgi0pUaE")
            ...
            if issue=="":
                ...
                return render_template('/patients/main.html',hospdata=hospdata,disease="")
            else:
                response = model.generate_content("I have a "+issue+", give ONLY email of best suited doctor from below list accordingly "+str(hospdata))
                response2 = model.generate_content("I have a "+issue+", give only one line 'name - description' of disease I may have")
                demail=response.text
                cursor.execute('Select * from docprofile Where demail=%s',(demail,))
                hospdata=cursor.fetchall()
                disease=response2.text
                return render_template('/patients/main.html',hospdata=hospdata,disease=disease)
        cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('Select * from docprofile')
        hospdata=cursor.fetchall()
...
def product():
    if request.method=='POST':
        ...
        return render_template('/patients/pharmacy.html',data=mdata)
    cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('select * from products')
    mdata=cursor.fetchall()
    return render_template('/patients/pharmacy.html',data=mdata)
...
def docmain():
    if 'loggedin' in session:
        ...
        cursor.execute('select dmeet from docprofile where demail=%s',(demail,))
...
def detailinfo(id):
    cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('Select * from docinfo where demail=%s',(id,)) 
...
def profileupdate():
    if request.method=='POST':
        ...
        # Create a cursor and update the record where demail matches admin_email
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute('''
                UPDATE docprofile
                SET dname = %s, dage = %s, demail = %s, dphno = %s, dphno2 = %s,
                    ddegree = %s, dspeci = %s, dclg = %s, drank = %s, dexp = %s,
                    dhosp = %s, dtreat = %s, dfees = %s, dmeet = %s, ddays = %s,
                    dtiming = %s, dstatus = %s
                WHERE demail = %s
            ''', (dname, dage, demail, dphno, dphno2, ddegree, dspeci, dclg, drank, dexp, dhosp, dtreat, dfees, dmeet, ddays, dtiming, dstatus, admin_email))
    ...
    cursor.execute("select * from docprofile where demail=%s",(docemail,))
    profiledata=cursor.fetchall()
    return render_template('/doctors/docprofileupdate.html',profiledata=profiledata)
